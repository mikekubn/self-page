...
const TopPanel = (): React.ReactElement => {
  ...
  return (
    <header className="fixed top-0 left-0 z-50 flex flex-row items-center px-7 bg-pink h-14 w-[100%]">
      <Navigation />
...
const PlaceholderLanguage = () => (
  <div className="flex flex-row items-center pl-4 cursor-pointer">
    <Image src="/image/translate.png" height={25} width={25} alt="Change language image" />
...
const ProvideImage = ({ val }: { val: boolean }) => {
  if (val) {
    return (
      <Image src="/image/moon.png" height={30} width={30} />
    );
  }

  return (
    <Image src="/image/sun.png" height={30} width={30} />
  );
};
